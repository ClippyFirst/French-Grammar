...
for (const name of fs.readdirSync(contentDir).filter((n) => n.endsWith('.md') || n.endsWith('.mdx')).sort()) {
  const source = fs.readFileSync(path.join(contentDir, name), 'utf8');
  const article = {
    path: 'src/content/fr/' + name,
    slug: name.replace(/\.(md|mdx)$/, ''),
    title: field(parseFrontmatter(source), 'title_uk'),
    status: parseStatus(source) ?? 'unknown',
  };
  for (const id of parseCanonicalIds(source)) {
    if (catalog.has(id)) catalog.get(id).articles.push(article);
  }
}
...
const links = (articles) => articles.length
  ? articles.map((a) => '[' + esc(a.title || a.slug) + '](../' + a.path.replace(/^src\//, '') + ')' + (a.status === 'deprecated' ? ' [deprecated]' : '')).join('<br>')
  : '—';
...
const lines = [
  ...
  '## Missing',
  '',
  ...(missing.length ? missing.map((r) => '- ' + tick + r.id + tick) : ['—']),
  '',
  '## Duplicate mappings',
  '',
  ...(duplicate.length ? duplicate.map((r) => '- ' + tick + r.id + tick + ' → ' + r.articles.map((a) => tick + a.slug + tick).join(', ')) : ['—']),
  '',
];
